...
class Gemini_interface:
    ...
    def chat(self, pdf: Union[str, List[str], None], text: str, max_tokens: int = 4096, history: Dict = None) -> tuple[str, Dict, float, float]:
        ...
        Args:
            ...
            history: Chat history (dict with 'cache' and 'content' keys, or empty list).

        Returns:
            Tuple of (response_text, updated_history, cost).
        """
        # 0. Normalize History Structure
        t0 = time.time()
        if not history:
            history = {"cache": None, "content": []}
        ...
        # Append messages to history content
        user_msg = {'role': 'user', 'parts': [text]}
        
        history["content"].append(user_msg)
        history["content"].append(model_msg)
    ...
    def save_history(self, history: Dict, dir_path: str):
        path = Path(dir_path)
        path.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = path / f"chat_history_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    if not key:
        ...
        hist = []
        try:
            res, hist, cost = gemini.chat(pdf=None, text="Hello, who are you?")
        ...
        if os.path.exists(pdf_path):
            ...
            try:
                ...
                res, hist, cost = gemini.chat(pdf=pdf_path, text="Summarize this document.", history=hist)
                print(f"Response: {res}")
                print(f"Cost: ${cost:.6f}")
            except Exception as e:
            ...
            try:
                ...
                res, hist, cost = gemini.chat(pdf=pdf_path, text="Another doc?", history=hist)
        ...
        try:
            ...
            res, hist, cost = gemini.chat(pdf=None, text="What was the summary again?", history=hist)
            print(f"Response: {res}")
            print(f"Cost: ${cost:.6f}")
        except Exception as e:
